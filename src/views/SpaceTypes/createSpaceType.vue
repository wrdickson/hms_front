...
<script lang="js">
export default {
  name: 'CreateSpaceType',
  emits: [ 'createSpaceType:create', 'createSpaceType:close' ],
  data () {
    return {
      cSpaceType: {
        title: '',
  ...
  methods: {
    createSpaceType ( createSpaceTypeRef ) {
      this.$refs[createSpaceTypeRef].validate( valid => {
        if( valid ) {
          this.$emit('createSpaceType:create', {...this.cSpaceType} )
        } else {
    ...
    close () {
      ...
      this.$emit('createSpaceType:close')
    },
